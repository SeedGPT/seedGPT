		...
		try {
			...
			const result = await this.workspace.runInWorkspace(async () => {
				...
				const searchCmd = isWindows 
					? `Get-ChildItem -Recurse -Include "*.ts","*.js","*.json","*.md" | Select-String -Pattern "${query}" | ForEach-Object { "$($_.Filename):$($_.LineNumber):$($_.Line)" }`
					: `grep -r --include="*.ts" --include="*.js" --include="*.json" --include="*.md" "${query}" .`
				
				const { stdout, stderr } = await execAsync(searchCmd, { shell: isWindows ? 'powershell.exe' : '/bin/bash' })
				return { stdout, stderr }
			})
		...
		try {
			...
			const result = await this.workspace.runInWorkspace(async () => {
				...
				const commands = isWindows ? [
					...
					'(Get-ChildItem -Recurse -Filter "*.test.ts" | Measure-Object).Count',
					'try { npm list --depth=0 2>$null } catch { "Package list unavailable" }',
					'try { git log --oneline -10 } catch { "Git history unavailable" }'
				] : [
				...
				for (const cmd of commands) {
					try {
						const { stdout } = await execAsync(cmd, { shell: isWindows ? 'powershell.exe' : '/bin/bash' })
