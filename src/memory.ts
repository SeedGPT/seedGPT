...
function getMemoryFiles (summariesDir: string): string[] {
	try {
		return fs.readdirSync(summariesDir)
			...
			.sort((a, b) => {
				const aTime = fs.statSync(path.join(summariesDir, a)).mtime
				const bTime = fs.statSync(path.join(summariesDir, b)).mtime
...
export async function updateMemory (memoryCfg: Config['memory'], summary: string, metadata?: Partial<MemorySummary['metadata']>): Promise<void> {
	try {
		...
		const markdownPath = memoryCfg.summaries + 'latest.txt'
...
async function archiveOldMemories (summariesDir: string): Promise<void> {
	try {
		...
		if (files.length > maxFiles) {
			const archiveDir = path.join(summariesDir, 'archives')
			const filesToArchive = files.slice(maxFiles)

			for (const file of filesToArchive) {
				const srcPath = path.join(summariesDir, file)
